...
class GradualWarmupScheduler(_LRScheduler):
    ...
    def get_lr(self):
        ...
        return [base_lr * ((self.multiplier - 1.) * self.last_epoch / (self.total_epoch+1e-5) + 1.) for base_lr in self.base_lrs]
...
def load_and_cache_withlabel(image_path,label_path,cache_file,image_size=0,shuffle=False):
    if cache_file is not None and os.path.exists(cache_file):
        ...
        for image_path,label in tqdm.tqdm(zip(images,labels),total=total_iterations):
            ...
            processed_image=preprocess_image(image_path,image_size=image_size) #only cgan
